  ...
  return (
    <>
      ...
      <Container>
        <Wrapper>
          <Popup
            trigger={<CreateButton >Create Quiz </CreateButton>}
            modal
            nested
          >
            {close => (
              <div style={{ fontSize: "12px", backgroundColor: "#393E46", width: "400px" }}>
                ...
                <form onSubmit={handleName}>
                  <div style={{ width: "100", borderBottom: "1px solid gray", fontSize: "18px", padding: "5px", color: "white" }}>New Quiz</div>
                  <div style={{ width: "100%", padding: "10px 5px" }}>
                    <input type="text" style={{ width: "90%", padding: "5px", borderRadius: "6px", border: "none" }} placeholder='Enter title for your quiz' onChange={e => setExamName(e.target.value)} required /><br />
                  </div>
                ...
                {examNameStorage.map((name) => (
                  ...
                  >
                    <TableCell component="th" scope="row" onClick={() => { navigator.clipboard.writeText("http://localhost:3000/quiz/" + name._id) }}>
                      <span style={{ cursor: "pointer" }} onClick={() => { notify(); }}> {name.examname}  <span style={{ color: "#CC0000",marginLeft: '.5rem'}}> Click for QUIZ link</span> </span>
                    </TableCell>
